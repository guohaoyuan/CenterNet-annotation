...
class model(kp):
    def __init__(self, db):
        ...
        super(model, self).__init__(
            db, n, 1, dims, modules, out_dim,               # modules是一个list，表示创建块的个数
            make_tl_layer=make_tl_layer,                    # 创建tl_pool，左上角池化
            make_br_layer=make_br_layer,                    # 创建右下角池化
            make_ct_layer=make_ct_layer,                    # 创建中心池化
            make_pool_layer=make_pool_layer,                # 创建一个空的seq，随后会添加最大值池化
            make_hg_layer=make_hg_layer,                    # 创建hourglass模块
